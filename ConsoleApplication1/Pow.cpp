...
void Pow::apply(Frame& frame, const Frame& original_frame, const size_t& width, const size_t& height, Process& process)
{
    ...
    double inv_w = scale / width;
    double inv_h = scale / height;
...
#pragma omp parallel for collapse(2)
    for (int y = 0; y < height; ++y)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            complexPow(&exp_y, &exp_x, pre_exp_x, pre_exp_y, real, imag, 0);

            // Re-map the coordinates
            int src_x = (exp_x + half_w) * width / scale;
            int src_y = (exp_y + half_h) * height / scale;
